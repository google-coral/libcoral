...
TEST(ComputeSumSquaredDistanceTest, SumDistanceIsZero) {
  std::vector<Point> embedding = {Point{0, 0}, Point{0.25, 0.75},
                                  Point{1.5, 0.50}};
  const PoseKeypoints pose = {
      {Point{0, 0}, Point{0.25, 0.75}, Point{1.5, 0.50}}};

  EXPECT_FLOAT_EQ(ComputeSumSquaredDistance(embedding, pose), 0.0f);
}

TEST(ComputeSumSquaredDistanceTest, SumDistanceIsNonzero) {
  std::vector<Point> embedding = {Point{0, 0}, Point{0.25, 0.75},
                                  Point{1.5, 0.50}};
  const PoseKeypoints pose = {
      {Point{0.5, 0.5}, Point{0.35, 0.65}, Point{0.4, 1.5}}};

  EXPECT_FLOAT_EQ(ComputeSumSquaredDistance(embedding, pose), 2.73f);
}

std::vector<float> GetLongOffsets(int height, int width, int num_keypoints) {
  // Create a long_offsets tensor with increasing values scaled from [0, 1]
  std::vector<float> long_offsets(height * width * 2 * num_keypoints);
  const int long_offsets_max_range =
      height - 1 + width - 1 + num_keypoints * 2 - 1;
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      for (int c = 0; c < num_keypoints * 2; ++c) {
        long_offsets[i++] =
            (y + x + c + 0.1f) / (long_offsets_max_range + 0.1f);
      }
    }
  }
...
TEST(GetEmbeddingTest, EmbeddingIsCorrect) {
  auto long_offsets =
      GetLongOffsets(/*height=*/10, /*width=*/8, /*num_keypoints=*/3);

  Point point_result = GetEmbedding(
      /*y_location=*/2, /*x_location=*/3, long_offsets.data(),
      /*keypoint_index=*/2, /*refinement_steps=*/10, /*height=*/10, /*width=*/8,
      /*num_keypoints=*/3, /*stride=*/16);

  EXPECT_NEAR(point_result.y, 131.8, 0.1);
  EXPECT_NEAR(point_result.x, 112.0, 0.1);
}

TEST(MatchEmbeddingToInstanceTest, InstanceIsIndexZero) {
  // Create a long_offsets tensor with increasing values scaled from [0, 1]
  auto long_offsets =
      GetLongOffsets(/*height=*/10, /*width=*/8, /*num_keypoints=*/3);
  constexpr int num_poses = 2;
  PoseKeypoints poses[num_poses] = {
      {Point{0, 0}, Point{0.25, 0.5}, Point{2, 0}},
      {Point{0, 0}, Point{0, 0}, Point{0, 0}}};

  EXPECT_EQ(MatchEmbeddingToInstance(
                /*y_location=*/2, /*x_location=*/3, long_offsets.data(),
                /*height=*/10,
                /*width=*/8, poses, num_poses, /*num_keypoints=*/3,
                /*refinement_steps=*/10, /*stride=*/16),
            0);
}

TEST(MatchEmbeddingToInstanceTest, InstanceIsIndexOne) {
  // Create a long_offsets tensor with increasing values scaled from [0, 1]
  auto long_offsets =
      GetLongOffsets(/*height=*/10, /*width=*/8, /*num_keypoints=*/3);
  constexpr int num_poses = 2;
  PoseKeypoints poses[num_poses] = {
      {Point{0, 0}, Point{0.25, 0.5}, Point{2, 0}},
      {Point{0.1, 0.2}, Point{0.9, 1}, Point{3, 1}}};
